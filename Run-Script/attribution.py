...
from distilllib.models.DNNClassifier import hgrn, varcnn
...
# 恢复预训练模型
teacher_model = hgrn(channels=channels, points=points,
                     num_classes=num_classes)  # DecMeg2014数据集上以FAKD效果最好的HGRN为例，CamCAN以VARCNN为例
# teacher_model = varcnn(channels=channels, points=points, num_classes=num_classes)  # DecMeg2014数据集上以FAKD效果最好的HGRN为例，CamCAN以VARCNN为例
...
pretrain_student_model_fakd_path = "../checkpoint/Models_Train/DecMeg2014_SDT_ShapleyFAKD-CE"
pretrain_student_model_fakd_ce_path = "../checkpoint/Models_Train/DecMeg2014_SDT_ShapleyFAKD"

# pretrain_teacher_model_path = "../checkpoint/Models_Train/CamCAN_VARCNN_20220616160458_checkpoint.pt"
# pretrain_student_model_vanilla_path = "../checkpoint/Models_Train/CamCAN_SDT_student"
# pretrain_student_model_fakd_path = "../checkpoint/Models_Train/CamCAN_SDT_ShapleyFAKD"
# pretrain_student_model_fakd_ce_path = "../checkpoint/Models_Train/CamCAN_SDT_ShapleyFAKD_CE"
...
M = 8
reference_num = 100
reference_dataset = data[-reference_num:]
for sample_id in range(sample_num - reference_num):
    origin_input, truth_label = data[sample_id], labels[sample_id]
    attribution_map_lists = shapley_fakd_parallel(origin_input, model_list, reference_dataset,
                                                  classes=classes, window_length=window_length, M=M)
    for model_id in range(len(model_list)):
        model = model_list[model_id]
        origin_pred = predict(model, origin_input.reshape(1, channels, points))
        ...
        result = AttributionResult(dataset, label_names, sample_id, origin_input, truth_label,
                                   model_name_list[model_id], origin_pred, origin_pred_label,
                                   attribution_map_lists[model_id])
...
for mean_class in [0, 1]:
    for model_id in range(len(model_list)):
        ...
        fig, heatmap_channel, top_channels = class_mean_plot(result_list, channels_info, label=mean_class,
                                                             top_channel_num=10)
